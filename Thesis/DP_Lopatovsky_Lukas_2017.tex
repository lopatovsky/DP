...
This Master's thesis provides a detail theoretical description of discrete-time and continuous-time hidden Markov models, it explains the HMMs algorithms with the emphases on continuous-time model learning methods. We have examined current state-of-the-art methods and chosen the most promising to be implemented into library. 

We have created the first of its art general purpose continuous-time hidden Markov model library under open-source license. The computationally effective and simple to use library provides broad functionality and various algorithms for both discrete-time and continuous-time model. Its main feature is EM learning algorithm using method of matrix exponentiation for counting end-state conditioned expectations (\textit{expm}).
...
We have examined the behaviour and characteristic of continuous-time hidden Markov model. 

descrube co sa robilo v thesach a knihovna
 
describ porovnanie s diskr a vyuzite cthhm
%malo informacie (vela hidden) menej presny.
%rychlo sa straca informacia

desrib varianty co a ako + full sparse and hunderts of states --- a vysledky testov zhrn.

interest before official release of the library
