...
\item \textbf{Expectation:} Calculate the expected value of the likelihood function $\mathrm{logP}(\mathbf{x},\mathbf{z};\hat \theta^{(0)})$. Where $\hat \theta^{(0)}$ is the current parameters estimation. We will find the function $\mathrm{Q}$ Count the probability under current parameters estimation.  
...
\item \textbf{Maximization:} Maximize the probabilities of parameters using their log-likelihood evaluation of previous step.  
\end{enumerate}  
