...
\begin{equation}\label{eq:CL2}
 CL = \prod_{i \in S} \prod_{j \in S, i \neq j} q_{ij}^{n_{ij} } e^{ - q_i \tau_i }
\end{equation}
...
\begin{equation}
 L = \prod_{v=0}^{V-1} \matr{P}_{y_v y_{v+1}}(\tau_v) 
\end{equation}

It can be alternatively extended in the form

\begin{equation}
 L = \prod_{v=0}^{V-1} \prod_{i,j \in S}  \matr{P}_{ij}(\tau_v)^{\mathbb{I}( y_v = i, y_{v+1} = j )} 
\end{equation}
...
\begin{equation}
 L = \prod_{\Delta = 1}^{r} \prod_{i,j \in S}  \matr{P}_{ij}(\tau_{\Delta})^{\mathbb{C}( \tau=\tau_{\Delta} y_v = i, y_{v+1} = j )} 
\end{equation}
