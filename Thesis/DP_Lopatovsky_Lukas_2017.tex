...
The Discrete-time Hidden Markov Model (HMM) is the doubly embedded stochastic process consisting of underlaying Markov process \ref{sec:DMP}, which actual states are not directly visible and only can be observed through the another process $O$. The process $O$ assigns to every state $s_i$ the probabilities that the specific observation symbol from the set $\mathcal{V}$ will be emitted. The set $\mathcal{V}=\{  v_1,v_2,\dots,v_m\}$ is the set of all observable, or also called emission symbols. The sequence of the symbols forms the known part of the model, and it can be used to guess the possible values of underlying states chain. The probability that the state $s_i$ emits the symbol $v_j$ will be denoted as $b_i(j)$ and together with other elements form the matrix $B$, the rectangular matrix of $n$ rows and $m$ columns. Comparing to the classic Markov process, HMM is able to model more complex systems, that can often be used as the description of the real life problems.  
  
To summarize the HMM can be described by its [zmen tu vetu, aj cely predosli odstavec :(]:
...
In general we do not know the state of the system during all the time. It is only known at some unevenly distributed times $\matr{T} = ( t_0, t_1, \dots, t_{v} )$ as $\matr{X}= (x_0, x_1, \dots, x_{v} )$. 
...
If the variable $r$ - the number of all distinct time intervals $\tau_{\Delta}, \Delta =\{1,2,\dots,r\}$ is lower then number of observations, it could be beneficial to aggregate them as in formula,
