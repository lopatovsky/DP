...
\section{Posterior...}\label{sec:pos}
%intro with 3 problems ..?
...
In previous section \ref{sec:pos}, we have showed, how to count $p(s(t_v) = k,s(t_{v+1}) = l \mid O,T, \hat Q_t)$. The remaining part of computing $\mathbf{E}(n_{ij} \mid s(t_v) = k,s(t_{v+1}) = l,\hat Q_t )$ and $\mathbf{E}(\tau_i \mid s(t_v) = k,s(t_{v+1}) = l,\hat Q_t )$ will be presented in [future ref]. 
