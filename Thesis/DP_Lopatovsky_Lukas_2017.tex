...
\abstractCS{V ňěkô n{\v e}kolika v{\v e}t{\' a}ch shr{\v n}te obsah a p{\v r}{\' i}nos t{\' e}to pr{\' a}ce v {\v c}esk{\' e}m jazyce.}
...
Giving the observation sequence (in the continuous-time model together with the time sequence) or even whole dataset of sequences, we can use the function to count the probability that it was  generated by given model. The function use the forward algorithm described in Section \ref{sec:fb}. Similar function takes additionally also the hidden state sequences and count the probability for all the sequences given the model.

The library enables us to generate our own data sequences according to model parameters. We can even create whole artificial dataset. That may be useful for experimenting with the models. 
The continuous time model also need to generate time sequences. We have chosen to generate them using the exponential distributed time intervals with the optional parameter $\lambda$ as it has characteristics similar to many real life events.  
...
Soft method and fast option set to true are used as default, because they are given best expected results and time efficiency \ref{sec:exper}.
...
\chapter{Experiments}\label{sec:exper}
...
For full matrix we have used models with 5 to 75 hidden states, for the sparse matrix models with 5 to 135 hidden states. Higher number of hidden states in sparse matrix couldn't be used because its training hist memory limit. We have used dataset of 1000 observation points.
...
In Figure \ref{fig:time} we can see that also the models with the high number of hidden states are able to converge in feasible time. Sparsity of the jump-rate matrix is crucial for the algorithm speed. It is possible to make three iteration in a hour by model with full matrix of 75 hidden states and approximately seven iterations by model with sparse matrix of 135 hidden states.   
...
\section{Future Work}
%rozsirenia - ina reprezentacia matic na sparse , continuous observations, vztah s rekurent neural networks.https://en.wikipedia.org/wiki/Recurrent_neural_network - Continuous time RNN. postupny trening na velke modely.
