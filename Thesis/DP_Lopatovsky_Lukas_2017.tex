...
There is no available analytic maximize of likelihood function \eqref{eq:CTL}, however it can be estimated iteratively by the EM algorithm proposed in [ref:PhysRevE.76].  
...
Now, there remain the non-trivial task to evaluate $\mathbf{E}( n_{ij} \mid Y, \hat Q_t )$ and $\mathbf{E}( \tau_i \mid Y, \hat Q_t )$.
