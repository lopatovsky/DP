...
Dividing it by time interval $t$ and letting $t$ decrease close to zero, we obtain the following equation \cite{Co77}. 
...
To accomplish the established goal we have chosen to create \textit{Python} library. Specifically, the development was done under \textit{Python 3.5}. The Python is highly used among machine-learning community and the library is easily accessible by downloading from \textit{Pip} \footnote{ \url{https://pypi.python.org/pypi/hmms} } or \textit{GitHub} \footnote{ \url{https://github.com/lopatovsky/HMMs} }.
...
The logarithmic grow of \textit{square and multiply} method is obvious. But to explain similar behavior of the $expm$ method we need to look deeper into its implementation \cite{al2009new}. 
...
We have created the first of its kind general purpose continuous-time hidden Markov model library under open-source license \footnote{ accesible at pip \url{https://pypi.python.org/pypi/hmms} and GitHub \url{https://github.com/lopatovsky/HMMs} }. The computationally effective and simple to use library provides broad functionality and implementation of current state-of-the-art algorithms for both discrete-time and continuous-time hidden Markov model. The most novel part are several variations of EM learning algorithm using method of matrix exponentiation for counting end-state conditioned expectations (\textit{expm}).
