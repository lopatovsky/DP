...
$B = \{ b_{ik} \}$, where $b_{ik}$ is the probability that the observation $k$ will occur, if the system is currently in state $i$. 
\begin{equation}
b_{ij} = \mathrm{P}(v_k \text{ at t } \mid q_t = S_i) \qquad 1 \leq i \leq N, \quad 1 \leq k \leq M
\end{equation}
...
When dealing with real-world application we need to deal with following problems, as proposed in [citace]. We will first describe the problems and later in separate subsections explain an efficient way haw to solve them. 
...
\subsection{first..}
\subsection{second..} there is more ways what can be considered optimal in this case ... more:[citate]
\subsection{third..}
